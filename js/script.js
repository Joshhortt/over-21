...
/* ****THREE BRANCHES****
What if we need to have more conditions? What if we want to display a special third message if the user is exactly 21?
Let's pop up an alert that says "Now don't go crazy!" if the age is equal to (===) 21:
else if is just like an alternate if. It also has a condition to be checked in parenthesis next to it,
and if that condition is true then the code in curly braces following it is run.
But else if's condition is only checked when the if condition has already been found to be false.
